...
var Crawler = function(initialURL) {
    ...
    crawler.prior = {}; // prior URL's

    function addPrior(queueItem) {
        crawler.prior[queueItem] = true; // URL encountered a problem
    }

    crawler.on("fetchtimeout", addPrior);
    crawler.on("fetchclienterror", addPrior);
//    crawler.on("fetchcomplete", addPrior);
    crawler.on("fetch404", addPrior);
    crawler.on("fetch410", addPrior);
...
Crawler.prototype.queueURL = function(url, referrer, force) {
    ...
    if (crawler.prior[queueItem]) { // previously failed request
//        return true; // seems like this should return false but tests require it to return true
        return !crawler.emit("queueduplicate", queueItem); // don't queue as it has a prior fetch problem; 404, etc
    }
